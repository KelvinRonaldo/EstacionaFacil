...
public class ContratoResource {
	...
	private Contrato getContratoMensalista(@PathVariable Long id){
		return contratoRepository.findByMensalista(id);
	}
	...
	@PostMapping
	private ResponseEntity<Contrato> salvarContrato(
			@RequestBody Contrato contrato,
			HttpServletResponse response){
		
		Integer vagas = contrato.getQuantidadeVagas();
		Double precoMensal = precoRepository.findByDataFimIsNull().getValorMensal();
		
		Double valorMensalista = precoMensal*vagas;
		
		contrato.setValorMensalista(valorMensalista);		
